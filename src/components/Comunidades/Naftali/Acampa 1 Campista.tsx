// /components/Comunidades/Naftali/Campista.tsx
...
const CACHE_KEY = "comunidade-naftali-acampa1-campistas-cache";
...
export function ComunidadeNaftaliAcampa1Campistas() {
  ...
  const fetchCampistas = async (force = false) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('Acampa1CampistaCarnaval2025')
        .select('*')
  ...
  const toggleStatus = async (campistaId: string, currentStatus: boolean | undefined) => {
  ...
  const downloadExcel = () => {
    ...
    const excelData = campistas.map((campista) => {
      ...
      return {
        ...
        'Nome': campista.nomeCompleto,
        'Email': campista.email,
        'CPF': campista.cpf,
                 'Nascimento': campista.dataNascimento ? formatDate(campista.dataNascimento) : '-',
        'Estado Civil': campista.estadoCivil,
        'Camiseta': campista.tamanhoCamiseta,
        'Profissão': campista.profissao,
        'Telefone': campista.telefoneContato,
        'Contato Emergência': campista.contatoEmergencia,
        'Tel. Emergência': campista.telefoneContatoEmergencia,
        'Cidade': campista.cidade,
        'Doença': campista.portadorDoenca ? 'Sim' : 'Não',
        'Especificar Doença': campista.doencas || '-',
        'Apto Atividades': campista.aptoAtividades ? 'Sim' : 'Não',
        'Alergia': campista.alergias ? 'Sim' : 'Não',
        'Especificar Alergia': campista.alergiasDetalhe || '-',
        'Medicação Contínua': campista.medicacao ? 'Sim' : 'Não',
        'Medicamentos': campista.medicamentos || '-',
        ...
        'Número Plano': campista.numeroInscricaoPlano || '-',
        'Responsabilidade': campista.responsabilidade ? 'Sim' : 'Não',
        'Autorização': campista.autorizo ? 'Sim' : 'Não',
                 'Data Inscrição': campista.createdAt ? formatDateTime(campista.createdAt) : '-'
      };
    });
    ...
    for (let row = 1; row <= excelData.length; row++) {
      for (let col = 0; col < Object.keys(excelData[0]).length; col++) {
        ...
        if (header === 'Nº') {
          ...
          ws[cellAddress].s.alignment = { ...ws[cellAddress].s.alignment, horizontal: "center" };
        } else if (['CPF', 'Telefone', 'Tel. Emergência'].includes(header)) {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Comunidade Naftali - Acampa 1");

    // Gerar e baixar arquivo Excel
    const fileName = `comunidade_naftali_acampa1_${new Date().toISOString().split('T')[0]}.xlsx`;
      ...
      <CardHeader className="bg-gradient-to-r from-indigo-50 to-purple-50 border-b px-6 py-5">
        <div className="flex flex-col gap-1">
          <CardTitle className="text-2xl font-bold text-slate-800">
            Comunidade Naftali - Acampa 1
          </CardTitle>
          <p className="text-sm text-slate-600">
            Acampa 1 - Inscrições Campistas 2025
          </p>
                ...
                campistas.map(campista => {
                  ...
                  return (
                    ...
                    >
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button
                                  size="sm"
                                  variant="outline"
                                  onClick={() => toggleStatus(campista.id, campista.ativo || false)}
                      ...
                      <TableCell className="py-3 px-4 font-medium">{campista.nomeCompleto}</TableCell>
                      ...
                      <TableCell className="py-3 px-4 text-slate-700">{campista.dataNascimento ? formatDate(campista.dataNascimento) : '-'}</TableCell>
                      <TableCell className="py-3 px-4 text-slate-700">{campista.estadoCivil || '-'}</TableCell>
                      <TableCell className="py-3 px-4 text-slate-700">{campista.tamanhoCamiseta || '-'}</TableCell>
                      <TableCell className="py-3 px-4 text-slate-700">{campista.profissao}</TableCell>
                      <TableCell className="py-3 px-4 font-mono text-xs">{campista.telefoneContato}</TableCell>
                      <TableCell className="py-3 px-4 text-slate-700">{campista.contatoEmergencia}</TableCell>
                      <TableCell className="py-3 px-4 font-mono text-xs">{campista.telefoneContatoEmergencia}</TableCell>
                      <TableCell className="py-3 px-4 text-slate-700">{campista.cidade}</TableCell>
                      <TableCell className="py-3 px-4 text-center">{formatBoolean(campista.portadorDoenca)}</TableCell>
                      <TableCell className="py-3 px-4 text-slate-700 max-w-[200px] truncate">{campista.doencas || '-'}</TableCell>
                      <TableCell className="py-3 px-4 text-center">{formatBoolean(campista.aptoAtividades)}</TableCell>
                      <TableCell className="py-3 px-4 text-center">{formatBoolean(campista.alergias)}</TableCell>
                      <TableCell className="py-3 px-4 text-slate-700 max-w-[200px] truncate">{campista.alergiasDetalhe || '-'}</TableCell>
                      <TableCell className="py-3 px-4 text-center">{formatBoolean(campista.medicacao)}</TableCell>
                      <TableCell className="py-3 px-4 text-slate-700 max-w-[200px] truncate">{campista.medicamentos || '-'}</TableCell>
                      ...
                      <TableCell className="py-3 px-4 text-slate-700 font-mono text-xs">{campista.numeroInscricaoPlano || '-'}</TableCell>
                      ...
                      <TableCell className="py-3 px-4">
                        {campista.comprovante ? (
                          <a href={campista.comprovante} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 py-1 px-3 bg-blue-50 text-blue-600 hover:bg-blue-100 hover:text-blue-700 rounded-md transition-colors duration-200 text-xs font-medium">
                            <Eye size={14} />
                      ...
                      <TableCell className="py-3 px-4 text-slate-700">{campista.createdAt ? formatDateTime(campista.createdAt) : '-'}</TableCell>
                    </TableRow>
                  );
                })
              )}
